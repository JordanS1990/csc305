                       ...
                       Vector3 m_Up)
{
    ...
    this->elements[8] = u.z;
    this->elements[9] = v.z;
    this->elements[10] = w.z;

    this->elements[12] = (-1) * (u.x*m_Eye.x) - (u.y*m_Eye.y) - (u.z*m_Eye.z);
    this->elements[13] = (-1) * (v.x*m_Eye.x) - (v.y*m_Eye.y) - (v.z*m_Eye.z);
    this->elements[14] = (-1) * (w.x*m_Eye.x) - (w.y*m_Eye.y) - (w.z*m_Eye.z);
    this->elements[15] = 1;

    //this->Translation(m_Eye);
}
                            ...
                            float m_Far)
{
    ...
    double f = 1 / (tan(m_Angle * M_PI / 180));

    this->elements[0] = f / m_AspectRatio;

    this->elements[5] = f;

    this->elements[10] = (m_Far + m_Near) / (m_Near - m_Far);
    this->elements[11] = -1;

    this->elements[14] = (2 * m_Far * m_Near) / (m_Near - m_Far);
    this->elements[15] = 0;
                      ...
                      float m_Far)
{
    ...
    this->elements[0] = 2 / (m_Right - m_Left);

    this->elements[5] = 2 / (m_Top - m_Bottom);

    this->elements[10] = -2 / (m_Far - m_Near);

    this->elements[12] = (-1) * (m_Right + m_Left) / (m_Right - m_Left);
    this->elements[13] = (-1) * (m_Top + m_Bottom) / (m_Top - m_Bottom);
    this->elements[14] = (-1) * (m_Far + m_Near) / (m_Far - m_Near);
}
